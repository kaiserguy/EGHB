...
import kaiserguy.eghb.HymnBook.Hymn;
	...
	public class HymnActivity extends Activity{
	    ...
	    protected void onCreate(Bundle savedInstanceState) {
	     ...
	     // Gesture detection
	        ...
	        theHymn = HymnBook.getInstance().getHymn(intent.getIntExtra("hymnNumber", 1));
	        text = theHymn.getHTML(defining);

	        //mWeb.setPivotX(0);
	        //mWeb.setPivotY(0);
	        mWeb.setBackgroundColor(Color.rgb(40,0,0));
	        hymnNumberView.setText(Integer.toString(theHymn.number));
	    ...
	    private OnClickListener onMeterClick = new OnClickListener() {
	    	public void onClick(View v) {
	        	Toast.makeText(getApplicationContext(), "Listing all hymns with a meter of " + ((TextView) v).getText(),
	    ...
	    private String getHymnURL(){
	    	String pageNumber = "1";
	    	int intHymnNumber = theHymn.number;
	    	...
	    	return "http://www.stempublishing.com/hymns/data/Dv1881_" + pageNumber + ".htm#" + intHymnNumber;
	    }
	    ...
	    private void launchTunes(int hymnNumber) {
	    	// Skip if no connection, or background data disabled
	    if (theHymn.meter.length() > 0){
	    ...
	    public void onDefineRequested(){
	    	if (defining){
	    		...
	    		if (isNetworkAvailable()){
		    		defining = true;
		    		String textDefined = HymnBook.getInstance().getHymn(getIntent().getIntExtra("hymnNumber",1)).getHTML(true);
	    ...
	    public void onUnDefineRequested(){
	    	...
	    	mWeb.loadDataWithBaseURL(null, "<html><head><META http-equiv='Content-Type' content='text/html; charset=UTF-8'><script language='javascript'>" + smoothScroll + "</script></head><body>" 
	    			+ text + "<hr style='width:15em;text-align:left;margin-left:2em;color:aaa;border-color:aaa;' /><hr style='width:13em;text-align:left;margin-left:3em;color:888888;border-color:888888;' /><br /><br /><br /></body></html>", "text/html", "utf-8", null);
	    }
	        ...
	        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
	            try {
	                ...
	                if(e1.getX() - e2.getX() > SWIPE_MIN_DISTANCE && Math.abs(velocityX) > SWIPE_THRESHOLD_VELOCITY) {
	            		...
	            		int hymnNumber = HymnActivity.this.theHymn.number;
	            		hymnNumber += 1;
